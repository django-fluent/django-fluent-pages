...
LANGUAGE_CHOICES = tuple([t for t in _languageChoices if t[0] in appsettings.ECMS_CODE_SHORTLIST])
if not appsettings.ECMS_CODE_SHORTLIST_ONLY:
    LANGUAGE_CHOICES += (_('Combinations'), [t for t in _languageChoices if '+' in t[0]]),
    LANGUAGE_CHOICES += (_('Advanced'), [t for t in _languageChoices if '+' not in t[0] and t[0] not in appsettings.ECMS_CODE_SHORTLIST]),
