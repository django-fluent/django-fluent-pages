...
class CmsPageDispatcher(GetPathMixin, View):
    ...
    def _intro_page(self):
        return TemplateResponse(
            self.request,
            "fluent_pages/intro_page.html",
            {"request": self.request, "site": Site.objects.get_current()},
        )
    ...
    def _try_appnode(self):
        ...
        sub_path = self.request.path[
            len(urlnode_path) :
        ]  # path_info starts at script_prefix, path starts at root.
    ...
    def _call_url_view(self, plugin, sub_path, match):
        ...
        # Get view response
        response = plugin.get_view_response(
            self.request, self.object, match.func, match.args, match.kwargs
        )
        if response is None:
            raise RuntimeError(
    ...
    def _is_own_view(self, match):
        return match.app_name == "fluent_pages" or match.url_name in (
            "fluent-page",
            "fluent-page-url",
        )
...
def _is_accidental_fallback(obj, requested_language):
    ...
    return getattr(obj, "_fetched_in_fallback_language", False) and obj.has_translation(
        requested_language
    )
